...
class JobRepository(IRepositoryAsync):
    ...
    async def create(self, job_create_dto: JobCreateDto) -> JobModel:
        ...
        return self._to_job_model(job_from_db=job, include_relations=False)
    ...
    async def retrieve_many(
        ...
        async with self.session() as session:
            ...
            res = await session.execute(query)
            job_from_db = res.scalars().all()

        job_models = []
        for job in job_from_db:
            model = self._to_job_model(
    ...
    async def update(self, id: int, job_update_dto: JobUpdateDto) -> JobModel:
        async with self.session() as session:
            ...
            title = (
                job_update_dto.title
                if job_update_dto.title is not None
                else job_from_db.title
            )
            description = (
        ...
        new_job = self._to_job_model(job_from_db, include_relations=False)
        return new_job
    ...
    async def delete(self, id: int):
        ...
        return self._to_job_model(job_from_db=job_from_db, include_relations=False)

    def _to_job_model(self, job_from_db: Job, include_relations: bool = False) -> JobModel:
        job_responses = []
        job_model = None
